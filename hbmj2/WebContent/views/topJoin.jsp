...
<script type="text/javascript">
	...
	function zipcheck(){			
		...
		open("postsearch.jsp","title","titlebar=no,left=600,top=100,width=600,height=600");
	}		
</script> 
</head>
<body id=login_center>
	<div id="login_title">MJ Hotel</div>
	<form name=signin method=get onsubmit="return check()" action=join.do>
	...
	<table id=login_tableF>
		<tr>
			...
			%>
			<td>
				<input type="text" name="id" placeholder="아이디 영문,숫자 4~12자리" value="<%=uu %>" class="ta">
				<input type=button value="ID중복확인" onClick="checkid()" class="bt">
				<div class="ta2">필수 정보입니다.<span class="ta3"><%=chch %></span></div>
				<div id=hr></div>
			</td>
		</tr>
		<tr>
			<td>
				<input type=password name=password1 placeholder=비밀번호 onblur="pass()" class="ta">
				<div class="ta2">필수 정보입니다.<span class="ta3" id="ch_pw1"></span></div>
				<div id=hr></div>
			</td>
		</tr>
		<tr>
			<td>
				<input type=password name=password2 placeholder="비밀번호 재확인" onblur="pass1()" class="ta">
				<div class="ta2">필수 정보입니다.<span class="ta3" id="ch_pw2"></span></div>
			</td>
		</tr>
	</table><p>
	...
	<table id=login_tableF>
		...
		<tr>
			<td>
			&nbsp;성별
				<input type="radio" id="radio1" name="gender" value="남" checked><label for="radio1">남자</label>
				<input type="radio" id="radio2" name="gender" value="여" ><label for="radio2">여자</label>
				<div id=hr></div>
			</td>
		</tr>
		<tr>
		...
		<tr>
		    <td>
			    &nbsp;주소
			    <input type=text name=zipcode placeholder=동이름 class="ta4">
			    <input type=button onClick="zipcheck()" value=우편번호 onLoad="send( )" class="bt">
		    	&nbsp;<input type=text name=addr class="ta"><br>
		    	<div id=hr></div>
		    </td>
 		</tr>
		<tr>
			<td><input type=text name=email placeholder="이메일" onblur="emailcheck()" class="ta">
			<div id=hr></div>
			</td>
		</tr>
		<tr>
			<td><input type=text name=tel placeholder="연락처" onblur="telcheck()" class="ta">
			<div id=hr></div>
			</td>
		</tr>
		<tr>
			<td>멤버십가입여부&nbsp;<input type=radio name=mem value="Membership" checked>Yes
				<input type=radio name=mem value="NON">No
			</td>
		</tr>
	</table>
	<div id=login_bottom>
		<input type=submit value="가입하기" class="bt2">
		<a href=../index.jsp><input type=button value="메인화면" class="bt2"></a>
	</div>
	</form>
