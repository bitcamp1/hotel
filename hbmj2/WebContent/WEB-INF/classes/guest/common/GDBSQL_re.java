...
public class GDBSQL_re {
	...
	public int boardInsert(DBbean_re bean){
		...
		}catch(Exception ex){ System.out.println("이용후기등록오류 : "+ex.toString()); }
		return success;
	}//insert END
	...
	public List<DBbean_re> boardSelect(String pnum){
		...
		try{
			...
			sql="select * from (select rownum rn, num, title, contents, writer, wdate, count, reco, pwd, cmtcount from reviewboard order by wdate desc) where rn between "+end+" and "+start;
			...
			while(RS.next()==true){
				...
				bean.setCmtcount(RS.getInt("cmtcount"));
				list.add(bean);
			}
		}catch(Exception ex){ System.out.println("이용후기 List 오류 : "+ex.toString()); }
		return list;
	}//select() END
	...
	public DBbean_re boardDetail(String nn){
		...
		}catch(Exception ex){ System.out.println("이용후기Detail 오류 : "+ex.toString());  }
		return bean;
	}//detail() END
	...
	public int cmtcountEdit(int ss){
		...
		int ok=0;
		try{
			sql="update reviewboard set cmtcount=? where num="+ss;
			System.out.println(sql);
			PST=CN.prepareStatement(sql);
		...
		}catch(Exception ex){ System.out.println("cmtcountEdit 실패 :" + ex.toString());  }
		return ok;
	}//eddit() END
	...
	public void dbDelete(String data1){
		try{
			sql="delete from reviewboard where num="+data1;
			ST=CN.createStatement();
			int ok=ST.executeUpdate(sql);
			if(ok>0){	System.out.println("삭제성공");  }
		}catch(Exception ex){ System.out.println("삭제실패 :"+ex.toString()); }
